...
画像の0,0からずらした場所をスタート地点として、8*8の離散コサイン変換の量子化によるぼかしをおこないます。何度かスタート地点を変えてぼかしを行い、その画像を重ねることで、ブロックノイズを消すことができます。なんでもffdshowのSSP deblockingもこのアルゴリズムを使用しているようです。
ただし、このSmoothD for AviUtlでは、速度と作者の趣味のため、直交変換にアダマール変換を使用しています。とは言え。本家のSmoothDはMMX化されたxvidのDCT-iDCTを使用しているのでそれなりに高速です。
...
マルチスレッド処理で画像にゴミが出るため、処理した画素データをwork_spaceに一時退避する処理に変更しました。メモリの同じ場所に同時に書き込もうとすると問題が出るようです。当然、なのかな? コンパイラが何とかしてくれるのではないかと、ちょっと希望は持っていたのですが。ただし。マルチスレッドで処理しない場合、まったく無駄な処理が一つ増えただけです。ごめんなさい。
...
int shift[] = {
 3, 1, 1, 4, 4, 7, 7, 3, 1, 1, 7, 1, 1, 7, 7, 7,
 5, 2, 7, 6, 0, 1, 1, 7, 6, 0, 4, 6, 0, 3, 4, 1,
 7, 3, 4, 3, 3, 7, 3, 0, 1, 4, 6, 4, 0, 5, 5, 1,
 5, 7, 0, 2, 7, 5, 0, 7, 4, 2, 2, 1, 7, 7, 1, 2,
 4, 5, 1, 6, 6, 2, 2, 3, 6, 6, 0, 4, 6, 1, 2, 6,
 3, 1, 4, 7, 1, 5, 5, 3, 1, 3, 5, 0, 3, 5, 6, 5,
 7, 0, 3, 2, 5, 6, 1, 1, 3, 6, 7, 4, 2, 0, 5, 4,
 6, 3, 2, 7, 3, 3, 4, 0, 2, 4, 4, 4, 7, 2, 0, 0,
};
...
TCHAR* track_name[] = {"threshold", "quant", "n_shift", "zero_weight",}; //	トラックバーの名前
int track_default[] = {24, 8, 5, 0,}; //	トラックバーの初期値
int track_s[] = {0, 1, 1, 0,}; //	トラックバーの下限値
int track_e[] = {255, 255, 6, 64,}; //	トラックバーの上限値

#define	CHECK_N	3	//　チェックボックスの数
TCHAR* check_name[] = {"Rounding", "All_Quantization", "DCT",}; //　チェックボックスの名前
...
FILTER_DLL filter = {
 ...
 nullptr, //	設定が変更されたときに呼ばれる関数へのポインタ (NULLなら呼ばれません)
 nullptr, //	設定ウィンドウにウィンドウメッセージが来た時に呼ばれる関数へのポインタ (NULLなら呼ばれません)
 nullptr, nullptr, //	システムで使いますので使用しないでください
 nullptr, //  拡張データ領域へのポインタ (FILTER_FLAG_EX_DATAが立っている時に有効)
 NULL, //  拡張データサイズ (FILTER_FLAG_EX_DATAが立っている時に有効)
 "SmoothD for AviUtl", //  フィルタ情報へのポインタ (FILTER_FLAG_EX_INFORMATIONが立っている時に有効)
...
BOOL func_proc(FILTER* fp, FILTER_PROC_INFO* fpip)
{
 ...
 const int zero_weight = fp->track[3]; //ノイズ除去後の画像に混ぜる元の画像の割合
 ...
 */
 ...
 /*
 作業領域に加算された値を変数 temp に合計、元画像をzero_weightだけ乗算して、12ビット化してから count で除算。
 */
 ...
 for (int y = 0; y < fpip->h; y++)
 {
  ...
  for (int x = 0; x < fpip->w; x++)
  {
   ...
   ycp->y = static_cast<short>(((ycp->y * zero_weight + temp.y) << 4) / count);
   ycp->cb = static_cast<short>(((ycp->cb * zero_weight + temp.cb) << 4) / count);
   ycp->cr = static_cast<short>(((ycp->cr * zero_weight + temp.cr) << 4) / count);
   ycp++;
   wsp++;
  }
 }
 ...
 for (int yblock = shifty; yblock < h; yblock += 8)
 {
   ...
   */
   ...
   for (int i = 1; i < 64; i++)
   {
    if (all_quantization)
    {
     if (abs(block[i]) < threshold) block[i] = (block[i] / quantization) * quantization;
     if (abs(blockCb[i]) < threshold) blockCb[i] = (blockCb[i] / quantization) * quantization;
     if (abs(blockCr[i]) < threshold) blockCr[i] = (blockCr[i] / quantization) * quantization;
    }
    else
    {
     if (abs(block[i]) < threshold) block[i] = block[i] / quantization * quantization;
     if (abs(blockCb[i]) < threshold) blockCb[i] = 0;
     if (abs(blockCr[i]) < threshold) blockCr[i] = 0;
    }
   }
   if (dct_on)
   {
